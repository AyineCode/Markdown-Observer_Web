...
**从 WSL 调 Windows 程序，必须给它一个 Windows 认的工作目录。**
当前目录是 WSL 路径时，Windows 那边看到的是 UNC（\`\\\\wsl.localhost\\...\`），
而 `cmd.exe` **拒绝**以 UNC 为当前目录——连 `%TEMP%` 都取不到。它阴在"看调用方"：
手工在终端里跑没事，从脚本（把 cwd 设成仓库路径）里跑就炸。
所以 `make-package.mjs` 里所有 `cmd.exe` / `csc.exe` 调用都带 `cwd: winCwd`
（WSL 下取 `/mnt/c`，Windows 下取 `C:\`，两边都认且不需要可写）。

**交叉打包时不要执行包里的 node。**
打包最后要跑 `node --version` 打印版本号，但 ARM64 的 node 在 x64 机器上必然 `EACCES`——
于是 `windows-arm64` 这一步**永远失败**，而且报错淹在几十行里看不出原因。
现在用 try/catch 兜住，打印"版本号略（本机跑不了 arm64 的 node）"。
